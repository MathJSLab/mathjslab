...
describe(`${unitName} unit test (.${testExtension} test file).`, () => {
    ...
    describe('Behavior', () => {
        ...
        it('Should provide signatures for all registered built-in functions.', () => {
            ...
            const moduleSignatureFailures = [
                ['Configuration', Configuration.functions, Configuration.signatures],
                ['CoreFunctions', CoreFunctions.functions, CoreFunctions.signatures],
                ['LinearAlgebra', LinearAlgebra.functions, LinearAlgebra.signatures],
            ].flatMap(([moduleName, functions, signatures]) => {
                const functionNames = Object.keys(functions as Record<string, unknown>);
                const signatureNames = Object.keys(signatures as Record<string, unknown>);
                const missing = functionNames.filter((name) => !(name in (signatures as Record<string, unknown>))).map((name) => `${moduleName}: missing signature for ${name}`);
                const orphan = signatureNames.filter((name) => !(name in (functions as Record<string, unknown>))).map((name) => `${moduleName}: orphan signature for ${name}`);
            ...
            expect(moduleSignatureFailures).toEqual([]);
