...
describe('MATLAB/Octave indexing compatibility fixtures.', () => {
    ...
    it('Should assign through chained cell-content indexing.', () => {
        ...
        expect(interpreter.Unparse(interpreter.Execute('C = {}; C(2) = {8}; C'))).toBe('C={[ ](0x0),8}\nC={[ ](0x0),8}\n{[ ](0x0),8}\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1}; C(2:3) = {8, 9}; C'))).toBe('C={1,8,9}\nC={1,8,9}\n{1,8,9}\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {}; C{2} = 8; C'))).toBe('C={[ ](0x0),8}\nC={[ ](0x0),8}\n{[ ](0x0),8}\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1}; C{3} = 8; C'))).toBe('C={1,[ ](0x0),8}\nC={1,[ ](0x0),8}\n{1,[ ](0x0),8}\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2, 3}; C(2) = []; C'))).toBe('C={1,3}\nC={1,3}\n{1,3}\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2, 3}; C([1, 3]) = []; C'))).toBe('C={2}\nC={2}\n{2}\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2, 3}; C{2} = []; C'))).toBe('C={1,[ ](0x0),3}\nC={1,[ ](0x0),3}\n{1,[ ](0x0),3}\n');
    ...
    it('Should assign to comma-separated cell contents through evaluated index lists.', () => {
        ...
        expect(interpreter.Unparse(interpreter.Execute(source))).toBe('C={7,0,9}\nidx=[1,3]\nC={7,0,9}\nC={7,0,9}\n{7,0,9}\n');
    });
    ...
    it('Should assign through chained structure-field indexing.', () => {
        ...
        expect(interpreter.Unparse(interpreter.Execute('U = []; U(2).x = 5; U.x'))).toBe(
            'U=[struct {\nx: [ ](0x0)\n},struct {\nx: 5\n}]\nU=[struct {\nx: [ ](0x0)\n},struct {\nx: 5\n}]\n[ ](0x0)\n5\n',
        );
    ...
    it('Should keep row, column, logical, and end-based indexing stable.', () => {
        ...
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3]; A([1, 2, 3]) = [9, 8, 7]; A'))).toBe('A=[9,8,7]\nA=[9,8,7]\n[9,8,7]\n');
        expect(interpreter.Unparse(interpreter.Execute('M = [1, 2; 3, 4]; M([1, 4; 2, 3]) = [10, 40; 30, 20]; M'))).toBe('M=[10,20;\n30,40]\nM=[10,20;\n30,40]\n[10,20;\n30,40]\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2}; C([1, 2]) = {9, 8}; C'))).toBe('C={9,8}\nC={9,8}\n{9,8}\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3, 4]; A([true, false, true, false]) = [9, 7]; A'))).toBe('A=[9,2,7,4]\nA=[9,2,7,4]\n[9,2,7,4]\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2, 3, 4}; C([true, false, true, false]) = {9, 7}; C'))).toBe('C={9,2,7,4}\nC={9,2,7,4}\n{9,2,7,4}\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3, 4]; A([1, 4]) = []; A'))).toBe('A=[2,3]\nA=[2,3]\n[2,3]\n');
        expect(interpreter.Unparse(interpreter.Execute('Column = [1; 2; 3; 4]; Column([1, 4]) = []; Column'))).toBe('Column=[2;\n3]\nColumn=[2;\n3]\n[2;\n3]\n');
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2, 3, 4}; C([1, 4]) = []; C'))).toBe('C={2,3}\nC={2,3}\n{2,3}\n');
        expect(interpreter.Unparse(interpreter.Execute('Column = [1; 2; 3; 4]; Column([true, false, true, false]) = []; Column'))).toBe('Column=[2;\n4]\nColumn=[2;\n4]\n[2;\n4]\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3; 4, 5, 6]; A(:, end)'))).toBe('A=[1,2,3;\n4,5,6]\n[3;\n6]\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3; 4, 5, 6]; A(end, :)'))).toBe('A=[1,2,3;\n4,5,6]\n[4,5,6]\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3; 4, 5, 6]; A(1, end) = 9; A'))).toBe('A=[1,2,9;\n4,5,6]\nA=[1,2,9;\n4,5,6]\n[1,2,9;\n4,5,6]\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3; 4, 5, 6]; A(:, end) = []; A'))).toBe('A=[1,2;\n4,5]\nA=[1,2;\n4,5]\n[1,2;\n4,5]\n');
        ...
        expect(interpreter.Unparse(interpreter.Execute('C = {1, 2, 3; 4, 5, 6}; C(:, end) = []; C'))).toBe('C={1,2;\n4,5}\nC={1,2;\n4,5}\n{1,2;\n4,5}\n');
        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3]; A(end+1) = 4; A'))).toBe('A=[1,2,3,4]\nA=[1,2,3,4]\n[1,2,3,4]\n');
        expect(interpreter.Unparse(interpreter.Execute('A = []; A(3) = 5; A'))).toBe('A=[0,0,5]\nA=[0,0,5]\n[0,0,5]\n');
    });

    it('Should keep compound indexed assignment on evaluated subscripts.', () => {
        const interpreter = Interpreter.Create();

        expect(interpreter.Unparse(interpreter.Execute('A = [1, 2, 3]; k = 2; A(k) += 10; A'))).toBe('A=[1,12,3]\nk=2\nA=[1,12,3]\n[1,12,3]\n');
    });
